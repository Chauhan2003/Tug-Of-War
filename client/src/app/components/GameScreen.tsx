...
export default function GameScreen() {
  ...
  // Robot AI — pulls rope right every 2.5-4.5s
  useEffect(() => {
    if (gameOver) return;
    const robotPull = () => {
      ...
      setTimeout(() => setPulling(null), 500);
    };
    const interval = setInterval(robotPull, 2500 + Math.random() * 2000);
  ...
  const handleSubmit = () => {
    ...
    if (correct) {
      ...
      setFeedback('wrong');
      setPulling('right');
      setRopePosition((prev) => Math.min(90, prev + 5));
      setStreak(0);
    }
